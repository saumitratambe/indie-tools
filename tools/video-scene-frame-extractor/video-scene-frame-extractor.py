...
def process_part(idx, start, end, root, video, logger):
    ...
    # 🔥 GPU ENCODE (Nvidia NVENC) -> OPTIMIZED FOR RTX 3060
    run([
        FFMPEG,  "-y ",
        "-ss ", str(start),
        "-to ", str(end),
        "-i ", str(video),
        "-an ",
        "-c:v ",  "h264_nvenc",        # <--- Uses your RTX 3060
        "-preset ", "p7",              # <--- Slower encode, better quality/speed balance
